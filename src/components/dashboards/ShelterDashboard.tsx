...
const ShelterDashboard = () => {
  ...
  return (
    <div className="animate-fade-in">
      <AppHeader subtitle="Shelter Dashboard" title={user?.name || "Shelter"} />
      ...
      <div className="px-4 -mt-4 space-y-4">
        <div className="grid grid-cols-2 gap-3">
          {[
            ...
            <button key={label} onClick={() => navigate(path)} className="flex items-center gap-3 rounded-xl border bg-card p-4 text-left transition-colors hover:bg-secondary">
              ...
              <div key={d.id} className="rounded-xl border bg-card p-4 space-y-3">
                ...
                {d.status === "Pending" && (
                  <div className="flex gap-2">
                    <Button size="sm" className="flex-1 gap-1" onClick={() => updateStatus(d.id, "Accepted")}><Check className="h-3.5 w-3.5" /> Accept</Button>
                    <Button size="sm" variant="outline" className="flex-1 gap-1 text-destructive border-destructive/30 hover:bg-destructive/10" onClick={() => { setDonations((prev) => prev.filter((don) => don.id !== d.id)); toast.info(`Donation ${d.id} declined`); }}><X className="h-3.5 w-3.5" /> Decline</Button>
                  </div>
                )}
                {d.status === "Accepted" && <Button size="sm" variant="secondary" className="w-full gap-1" onClick={() => updateStatus(d.id, "Collected")}>Mark as Collected</Button>}
                {d.status === "Collected" && <Button size="sm" variant="secondary" className="w-full gap-1" onClick={() => updateStatus(d.id, "Fed")}>Mark as Fed</Button>}
              </div>
            ))}
          </div>
        </div>
            ...
            {mockReports.map((r) => (
              <div key={r.id} className="rounded-xl border bg-card p-4">
                <div className="flex items-start justify-between">
                  ...
                  <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${r.status === "Open" ? "status-pending" : r.status === "In Progress" ? "status-accepted" : "status-fed"}`}>{r.status}</span>
                </div>
